         ...
         <div class="modal-dialog modal-dialog-centered mw-650px">
             <div class="modal-content rounded">
                 ...
                 <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                     ...
                     <form action="{{ route('user-validator-roles.update', $item->id) }}" method="POST" class="form">
                         @csrf
                         @method('PUT')
                         ...
                         <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                             <div class="col-md-12 fv-row fv-plugins-icon-container">
                                 <label class="required fs-6 fw-bold mb-2">Validator Role</label>
                                 <select class="form-select" name="validator_role_id" data-control="select2"
                                     data-placeholder="Select an option">
                                     <option value="" data-select2-id="select2-data-63-6qoa">Select validator role...
                                     </option>
                                     @foreach ($validatorRoles as $validatorRole)
                                         <option value="{{ $validatorRole->id }}"
                                             {{ $item->validator_role_id == $validatorRole->id ? 'selected' : '' }}>
                                             {{ $validatorRole->nama_validator }}
                                         </option>
                                     @endforeach
                                 </select>
                                 <div class="fv-plugins-message-container invalid-feedback"></div>
